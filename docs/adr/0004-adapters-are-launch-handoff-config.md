...
interfaces) changes faster than any integration code could track. Velora's
...
packet template, and notes. Velora's MVP performs no model API calls — handoff is via
...
- Harder: no automated round-trips — a human (or the agent itself, via files) closes
  the loop. Accepted: Velora is a routing and record layer, not an orchestrator.
...
- **Provider SDK integrations** — rejected for MVP: constant drift, key management,
  and it pulls Velora toward becoming another chat interface (an explicit anti-goal).
